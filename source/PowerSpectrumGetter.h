...
template <typename T>
struct PffftAllocator : std::allocator<T>, protected PffftAllocatorBase
{
   PffftAllocator()
   {
   }
...
struct PffftAlignedCount
{
   ...
   explicit PffftAlignedCount(size_t nFloats)
       : value { ((nFloats + (FloatAlignment)-1) / FloatAlignment) *
                 FloatAlignment }
   {
   }
...
inline auto operator*(PffftAlignedCount x, Integral y) -> std::enable_if_t<
...
template <typename Integral>
inline auto operator*(Integral x, PffftAlignedCount y) -> std::enable_if_t<
   std::is_unsigned_v<Integral> && sizeof(Integral) <= sizeof(size_t),
   PffftAlignedCount>
{
